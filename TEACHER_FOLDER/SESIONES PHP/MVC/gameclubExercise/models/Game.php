<?php
/* Esta clase va a gestionar la tabla `videojuegos` */
...
class Game
{
  ...
  {
    $consultaSQL = "SELECT videojuegos.id, videojuegos.nombre, videojuegos.descripcion, generos.nombre AS genero, desarrolladores.nombre AS desarrollador, plataformas.nombre AS plataforma, pegui.descripcion AS pegui, videojuegos.fechaPublicacion, videojuegos.isoCode, videojuegos.imagen, tarifas.tipo AS tarifa, videojuegos.precio
    FROM videojuegos 
    INNER JOIN generos ON videojuegos.id_genero = generos.id 
    INNER JOIN desarrolladores ON videojuegos.id_desarrollador = desarrolladores.id
    INNER JOIN plataformas ON videojuegos.id_plataforma = plataformas.id
    INNER JOIN pegui ON videojuegos.id_pegui = pegui.id
    INNER JOIN tarifas ON videojuegos.id_tarifa = tarifas.id;"; // Aquí saco todos los productos y sus Foreign Keys asociados
  ...
  public function getByIDPathVariable($id)
  {
    ...
    WHERE videojuegos.id = $id;"; // Aquí saco el producto a través de su ID y sus Foreign Keys asociados
