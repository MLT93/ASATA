...
class Usuario
{
    ...
    {
        $cnx = new \mysqli(DB_HOSTT, DB_USERR, DB_PASSS, DB_NAME);
    ...
    {
        $cnx = new \mysqli(DB_HOSTT, DB_USERR, DB_PASSS, DB_NAME);
        $SQL = "SELECT hashedPassword FROM usuarios WHERE usuarios.email= '$email'";
        $stmt = $cnx->query($SQL);
        // $respuesta = $stmt->fetch_all(MYSQLI_ASSOC); // Matriz asociativa
    ...
    public static function actualizarPassword(string $email, string $password)
    {
        ...
        $SQL = "UPDATE usuarios SET hashedPassword = '$hashedPassword' WHERE usuarios.email = '$email' ";
        $cnx->query($SQL);
    }
