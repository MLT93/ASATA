...
class User
{
  ...
  {
    $consultaSQL = "SELECT * FROM users;"; // Aquí saco toda la información necesaria. Además de la info entre las tablas relacionadas por sus Foreign Keys
  ...
  public function add($username, $pass, $email, $created_at = "") // Al tener en la DB una función que por defecto toma la fecha de registro, dejo `$created_at` con un valor por defecto vacío
  {
    // $consultaSQL = "INSERT INTO users (username, pass, email, created_at) VALUES (?, ?, ?, ?);"; // Para preparar esta consulta, los valores vacíos de VALUES deben escribirse como `?` porque utilizamos la class `\mysqli`
    ...
    */
    // $consultaPrepare->bind_param("ssss", $username, $pass, $email, $created_at);

    // return $consultaPrepare->execute(); // Ejecuto la consulta

    $consultaSQL = "INSERT INTO users (username, pass, email, created_at) VALUES ('$username', '$pass', '$email', '$created_at');";
  ...
  {
    $id = intval($_GET['id']);
    $consultaSQL = "SELECT * FROM users WHERE id = $id;"; // Aquí saco la info a través de su ID y sus Foreign Keys asociados
