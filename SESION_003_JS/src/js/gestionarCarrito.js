...
buttonCarrito.onclick = function () {
  ...
  do {
    ...
    let precioUnitarioProducto = Number(prompt(`Introduzca el precio unitario de ${nombreProducto}`));
    // `isNaN()` comprueba si es un `Number` o no, devolviendo `true` o `false`
    if (precioUnitarioProducto <= 0 || isNaN(precioUnitarioProducto)) {
      alert('El precio ingresado no es válido. Por favor, ingrese de nuevo el precio.');
      // `continue` Esta instrucción salta la iteración actual y pasa directamente a la siguiente.
      continue;
    }
    let cantidadProducto = Number(prompt(`Introduzca cantidad de unidades del producto ${nombreProducto}`));
    if (cantidadProducto <= 0 || isNaN(cantidadProducto)) {
      alert('La cantidad ingresada no es válida. Por favor, ingrese nuevamente la cantidad.');
      continue;
    }
